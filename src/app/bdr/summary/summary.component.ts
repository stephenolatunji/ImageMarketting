...
export class SummaryComponent implements OnInit {
  ...
  parameter: any; header: any; subHeader: any; parameter2: any; tasks: any; _image: any; show: boolean = false; data;
  ...
  setHeader() {
    const headers = ['Chiller Verification', 'Trophy Stout Price Poster', 'Trophy Lager Price Poster'];
  ...
  getTask() {
    ...
    if(this.tasks!==null) {
      ...
      switch (this.parameter2) {
        ...
        case 3:
          (this.parameter == 0)? 
          this.tasks = this.tasks.filter(task => task.taskType == 'hero_pp') :
          this.tasks = this.tasks.filter(task => task.taskType == 'hero_pp' && task.action=='success');
          break;
